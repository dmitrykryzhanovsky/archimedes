...
{
    /// <summary>
    /// Бинарная невозрастающая ирамида.
    /// </summary>
    public class BinaryMaxHeap<TKey, TValue> : MaxHeap<TKey, TValue> where TKey : INumber<TKey>
    {
        #region Constructors

        public BinaryMaxHeap () : base (BinaryDimension)
        {
        }

        public BinaryMaxHeap (List<TKey> keys, List<TValue> values) : base (BinaryDimension, keys, values)
        {
        }

        public BinaryMaxHeap (params (TKey key, TValue value) [] items) : base (BinaryDimension, items)
        {
        }
