namespace Archimedes
{
    public static class MathConst
    {
        public const double DegToRad = double.Pi / 180.0;

        public const double MinToRad = double.Pi / (180.0 * 60.0);

        public const double SecToRad = double.Pi / (180.0 * 60.0 * 60.0);

        public const double RadToDeg = 180.0 / double.Pi;

        public const double RadToMin = 180.0 * 60.0 / double.Pi;

        public const double RadToSec = 180.0 * 60.0 * 60.0 / double.Pi;

        public const double M_PI_2 = double.Pi / 2.0;
    }
}